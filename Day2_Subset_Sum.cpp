class Solution {
  public:
    bool solve(int i , vector<int>& w , int sum , vector<vector<int>>&t ){
        if(sum == 0) return true;
        if(i==0) return (w[0] == sum);
        ...
        if(w[i] <= sum) {
        return t[i][sum] = solve(i - 1, w, sum - w[i],t) || solve(i - 1, w, sum,t);
        }
        else if(w[i] > sum){
            return t[i][sum] = solve(i-1,w,sum,t);
        }
    }
    bool isSubsetSum(vector<int>& arr, int target) {
        int n = arr.size();
        vector<vector<int>>t(n+1,vector<int>(target+1,-1));
        return solve(n-1 , arr , target , t);
    }
};

/* Time Complexity Analysis
   : O(n*sum) where sum is the ‘target sum’ and 'n' is the size of an array

    Auxiliary Space: O(sum*n) + O(n), as the size of 2-D array is sum*n + O(n) extra space .
